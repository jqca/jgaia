...
# ⛔ officer は「寺園 諒雅」で確定（2026-08-17 社長ご確認）。⛔「涼雅」に戻さない。
#    当時 社内資料が「涼雅」で割れていたが、公開中の法定表示（qai-zen.com/legal）
#    と当サイトの「諒雅」が正しく、社内側 15か所を諒雅に揃えた。
SELLER = {
    ...
    # ⛔ 口座番号をこのファイルに直書きしないこと（2026-08-17）。
    #    jqca/jgaia は **公開リポジトリ**で、匿名でも読める（実測 HTTP 200）。
    #    請求書に載る情報ではあるが、公開の場に置くと「口座が変わりました」型の
    #    振込詐欺の材料になり、しかも履歴に残るので後から消せない。
    #    値は環境変数 SELLER_BANK（Railway）で渡す。
    # ⛔ 空のときは invoice_data() が発行を止める（お客様が払えないため）。
    'bank': os.environ.get('SELLER_BANK', ''),
    # 適格請求書発行事業者の登録番号（T＋13桁）。2026-08-17 社長よりご教示。
    # 国税庁の法人番号公表サイトで照合済み＝法人番号 2010001240988 は
    # 株式会社ＺｅｂｒａＱｕａｎｔｕｍ（所在地も上記 address と一致）。
    # ⛔ 公表情報なのでここに置いてよい（口座番号とは扱いが違う）。
    # ⛔ 未登録の団体の番号を借りて書かないこと。
    'invoice_no': 'T2010001240988',
    'tel': '',
}
...
def invoice_data(booking_id):
    ...
    bank = (SELLER.get('bank') or '').strip()
    if not bank or '要記入' in bank:
        return None, ('お振込先が未登録のため請求書を発行できません'
                      '（環境変数 SELLER_BANK をご設定ください）')
